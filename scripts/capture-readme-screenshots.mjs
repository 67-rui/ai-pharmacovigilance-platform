...
const port = Number(process.env.SCREENSHOT_PORT ?? 3102);
...
const rankingFixture = {
  ...
  assumptions: [
    "Rows are ordered by interpretation class, report count, PRR, and ROR.",
    "Ranking supports reviewer prioritization, not clinical risk ranking.",
  ],
};
...
function startServer() {
  return spawn("npm", ["--workspace", "apps/web", "run", "dev", "--", "-p", String(port)], {
    cwd: repoRoot,
    env: {
      ...process.env,
      FORCE_COLOR: "0",
...
try {
  ...
  await page.screenshot({
    path: path.join(outputDir, "dashboard-overview.png"),
    animations: "disabled",
    fullPage: false,
  });
  await capture(page.getByTestId("ai-report"), "ai-report.png");
  await capture(page.getByTestId("signal-analysis"), "signal-analysis.png");
  await capture(page.getByTestId("drug-comparison"), "drug-comparison.png");
  await capture(page.getByTestId("source-provenance"), "source-provenance.png");
