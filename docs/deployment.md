...
| `DEEPSEEK_API_KEY` | No | Enables DeepSeek medication-label extraction. | `/api/intake/medication` returns deterministic local extraction with the same schema. |
| `DEEPSEEK_MODEL` | No | Selects the DeepSeek chat model. | Defaults to `deepseek-chat`. |

Do not commit `.env.local` or plaintext API keys. The app is designed so a portfolio reviewer can still see the core workflow without OpenAI or DeepSeek keys.
...
- **OCR is poor on low-quality images:** paste corrected label text manually before running intake.
